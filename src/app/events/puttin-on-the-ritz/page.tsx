...
export default function Home() {
    const [event, setEvent] = useState<EventResponse | null>({
        images: 0,
        eventData: events["puttin-on-the-ritz"]
    });
    ...
    return (
        <>
            <Lightbox
                ...
                close={() => setIndexlb(-1)}
                index={indexlb}
                slides={event ? [...(Array.from({ length: event.images }, (_, i) => i + 1)).map((image, index) => { return { src: '/images/puttin-on-the-ritz/'+(image)+'.webp', alt: 'Image '+(image) }; })] : []}
            />
                            ...
                            <SmallCentered id="images">
                                ...
                                <div className="cosume-ideas-images">
                                    {(Array.from({ length: event.images }, (_, i) => i + 1)).map((image, index) => {
                                        return <div onClick={()=>{
                                            setIndexlb(index);
                                        }} className="idea-image" style={{backgroundImage:`url("/images/puttin-on-the-ritz/${(image)}.webp")`}}></div>
                                    })}
                                </div>
                            </SmallCentered> 
                        </div>
